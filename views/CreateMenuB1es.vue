<template>
  ...
  <h1 v-if="show"> Crear carta Bakkano 1 (ES)</h1>
   </transition >
  <transition name="show2">
...
<!--added -->
   ...
   <div  id="eventContainer" v-if="documents">
        ...
        <router-link class="links" :to="{name:'B1DeAdES', params:{ id:doc.id}}">
...
show.value=true
  ...
  setTimeout(showw, 0)
    ...
    const {addDoc, error} = useCollection('menuB1Es')
   ...
   console.log(title.value)
    ...
    //added
    const {  documents  } = getCollections('menuB1Es')
