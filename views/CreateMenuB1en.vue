<template>
  ...
  <h1 v-if="show"> Create menu Bakkano 1 (EN)</h1>
  </transition >
  <transition name="show2">
...
<!--added -->
   ...
   <div  id="eventContainer" v-if="documents">
        ...
        <router-link class="links" :to="{name:'B1DeAdEN', params:{ id:doc.id}}">
...
show.value=true
  ...
  setTimeout(showw, 0)
    ...
    const {addDoc, error} = useCollection('menuB1En')
   ...
   console.log(title.value)
    ...
    //added
    const {  documents  } = getCollections('menuB1En')
