<template>
   <router-link id="nav" :to="{name:'CreateMenuB1en'}"><button class="btn" >Back</button></router-link>
...
import AddItems1es from '@/components/AddItems1es.vue'
...
export default {
props:['id'],
components:{AddItems1es},
setup(props){
      const show2 = ref(false)
  const show3 = ref(true)
 const router = useRouter()
        const { error4, document:cate } = getDocument('menuB1Es', props.id)
        const {  isPending, error5 ,deleteDoc} = useDocument('menuB1Es',props.id)
        const {error7, document:doc} = getEvent('menuB1Es',props.id)
        const { deleteImg } = picStorage()
          const handleDelete = async () =>{
          await deleteImg(doc.value.filePath)
          await deleteDoc()
          console.log('deleting...')
    router.push({name:'CreateMenuB1es'} )

      }
const { updateDoc } = useDocument('menuB1Es',props.id)
