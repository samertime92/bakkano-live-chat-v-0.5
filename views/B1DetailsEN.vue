<template id="suck">
<div id="container1">
<img  @click="scrollMe" id="arrow" src="../assets/arrow2.png" alt="">

<img  @click="scrollMe2" id="arrow3" src="../assets/arrow2.png" alt="">

<div class="navbar">
      <nav>

          <!-- <h1><router-link class="links" :to="{ name: 'Chatroom'}">Chatroom</router-link></h1> -->
          <h4><router-link class="links3" :to="{ name: ''}">Reserve</router-link></h4>
          <h4><router-link  class="links3" :to="{ name: 'Welcome'}">Home</router-link></h4>

          <h4><router-link class="links3" :to="{ name: ''}">Community</router-link></h4>
...
<div  v-for="item in cate.items" :key="item.id">
 ...
 <div id="menuStuff"  v-if="documents">
          <div  v-for=" doc in formattedDocuments" :key="doc.id">
        <router-link class="links"  :to="{name:'Mirrorb1EN', params: {id:doc.id} } ">
...
import Bakkano1menuEN from '../components/Bakkano1menuEN.vue'
...
export default {
props:['id'],
components:{AddItems,Bakkano1menuEN},
setup(props,context){

// var scrollY = document.querySelector(#'');
// scrollY=x
...
console.log(route)
    ...
    const { error4, document:cate  } = getDocument('menuB1En', route.params.id)
...
nav{ display :flex; align-items :center ; max-width:1200px; margin:0 auto;}
...
.links3{ z-index:3;background-color:white;font-weight:bolder;font-size:15px;color:red;text-decoration:none;font-family: monospace;border:2px;border-radius: 10px;border-color:red;border-style: solid;padding: 5px;box-shadow:2px 4px  6px rgba(47,79,79,0.5) }
.links3:hover{background-color:rgb(255, 255, 255);color:red;transition: 0.5s;}



h4{color:white ; display:inline-block;background-color:rgb(255, 0, 0);border-radius:20px; box-shadow: 2px 10px 10px rgba(85, 83, 83,0.350)}
...
h1{ filter: brightness(170%); text-decoration: none;
 ...
 h3{color:rgba(255, 72, 72, 0.972);font-size: 15px;}
...
h2{ filter: brightness(170%);
  font-size: 20px;
  background: -webkit-linear-gradient(rgb(100, 1, 1), rgba(207, 0, 0, 0.719));
  -webkit-background-clip: text;
  -webkit-text-fill-color: transparent;}
</style>