<template>
  ...
  <h1 v-if="show"> Crear carta Bakkano2 (ES)</h1>
  </transition >
  <transition name="show2">
...
<div v-if="show">
  ...
  <div class="error"></div>
  <button v-if="!isPending">Crear</button>
...
<!--added -->
   ...
   <div  id="eventContainer" v-if="documents">
        ...
        <router-link class="links" :to="{name:'B2DeAdES', params:{ id:doc.id}}">
...
show.value=true
  ...
  setTimeout(showw, 0)
    ...
    const {addDoc, error} = useCollection('menuB2Es')
   ...
   console.log(title.value)
    ...
    //added
    const {  documents  } = getCollections('menuB2Es')
