...
import AddItems1 from '@/components/AddItems1.vue'
...
export default {
props:['id'],
components:{AddItems1},
setup(props){
    const show2 = ref(false)
  const show3 = ref(true)
 const router = useRouter()
        const { error4, document:cate } = getDocument('menuB1En', props.id)
        const {  isPending, error5 ,deleteDoc} = useDocument('menuB1En',props.id)
        const {error7, document:doc} = getEvent('menuB1En',props.id)
        const { deleteImg } = picStorage()
          const handleDelete = async () =>{
          await deleteImg(doc.value.filePath)
          await deleteDoc()
          console.log('deleting...')
    router.push({name:'CreateMenuB1en'} )

      }
const { updateDoc } = useDocument('menuB1En',props.id)
