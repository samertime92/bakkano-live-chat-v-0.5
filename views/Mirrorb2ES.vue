...
<div  v-for="item in cate.items" :key="item.id">
 ...
 <div v-if="documents">
        ...
        <router-link class="links"  :to="{name:'B2DetailsES', params: {id:doc.id} } ">
...
import Bakkano2menuES from '../components/Bakkano2menuES.vue'
...
export default {
props:['id'],
components:{AddItems,Bakkano2menuES},
setup(props,context){
...
console.log(route)
    ...
    const { error4, document:cate  } = getDocument('menuB2Es', route.params.id)
...
// }
    ...
    //   const  {user} = getUser()
  const { error, documents  } = getCollections('menuB2Es')
...
h4{color:white ; display:inline-block;background-color:black;border-radius:10px}
...
transition: all ease 0.2s; background: linear-gradient(0deg, rgba(243, 239, 239, 0.979), rgba(88, 4, 84, 0.192), rgba(0, 0, 0, 0))}
