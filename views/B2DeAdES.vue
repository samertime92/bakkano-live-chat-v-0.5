<template>
      <router-link id="nav" :to="{name:'CreateMenuB2es'}"><button class="btn" >Back</button></router-link>
...
import AddItems2es from '@/components/AddItems2es.vue'
...
export default {
props:['id'],
components:{AddItems2es},
setup(props){

    const show2 = ref(false)
  const show3 = ref(true)
 const router = useRouter()
        const { error4, document:cate } = getDocument('menuB2Es', props.id)
        const {  isPending, error5 ,deleteDoc} = useDocument('menuB2Es',props.id)
        const {error7, document:doc} = getEvent('menuB2Es',props.id)
        const { deleteImg } = picStorage()
          const handleDelete = async () =>{
          await deleteImg(doc.value.filePath)
          await deleteDoc()
          console.log('deleting...')
    router.push({name:'CreateMenuB2es'} )

      }
const { updateDoc } = useDocument('menuB2Es',props.id)
