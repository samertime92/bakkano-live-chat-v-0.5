...
<div class="navbar">
      <nav>
          ...
          <h4><router-link class="links3" :to="{ name: ''}">Reserva</router-link></h4>
          <h4><router-link  class="links3" :to="{ name: 'WelcomeEs'}">Inicio</router-link></h4>

          <h4><router-link class="links3" :to="{ name: ''}">Communidad</router-link></h4>
...
<div  v-for="item in cate.items" :key="item.id">
 ...
 <div v-if="documents">
        ...
        <router-link class="links"  :to="{name:'Mirrorb1ES', params: {id:doc.id} } ">
...
import Bakkano1menuES from '../components/Bakkano1menuES.vue'
...
export default {
  emits:["keep"],
props:['id'],
components:{AddItems , Bakkano1menuES ,  Welcome },
setup( props ) {
...
console.log()
    ...
    const { error4, document:cate  } = getDocument('menuB1Es', route.params.id)
...
// }
    ...
    //   const  {user} = getUser()
  const { error, documents  } = getCollections('menuB1Es')
