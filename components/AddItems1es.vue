...
<input type="text" required placeholder=" Titulo" v-model="title">
<input type="text" required placeholder="Descripción" v-model="desc">
<input type="text" required placeholder="Precio" v-model="price">
<input type="text" required placeholder="Alérgenos" v-model="allerg">
<button>Add</button>
</form>
</div>
</div>
...
export default {
    props:['item'],
    setup(props){
            const { error5 , updateDoc } = useDocument('menuB1Es', props.item.id)
    const title = ref('')
    const desc = ref('')
    const price = ref('')
    const allerg = ref('')
